...
func (s *Server) requestAuth(method string, path string, data []byte, headers map[string]string) ([]byte, error) {
...
func (s *Server) requestValidation(jwt string) (*ValidationResp, error) {
	...
	auth, err := s.requestAuth(http.MethodPost, "api/v1/auth/validate/", marshal, nil)
	if err != nil {
		return nil, err
	}
